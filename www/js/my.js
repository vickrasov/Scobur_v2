...
$(function(){	
	...
	$(document).on('click', 'a[data-role=view_one_user]', function(){
		var id=$(this).attr('data-id');
			$.post("http://zonar.co/Zonar/one_user.php", {z1:id}, function(res){
				...
				//then info to distance
				$.ajax({
				  type: 'POST',
				  url: "http://maps.googleapis.com/maps/api/distancematrix/json?origins="+sessionStorage.lat+","+sessionStorage.lng+"&destinations="+info[0].x1+","+info[0].y1+"&mode=driving&sensor=false&units="+sessionStorage.meter,
				  data: '',
				  success: function(response){
					  if (response.rows[0].elements[0].status=='OK'){
						kiko='Distance: '+response.rows[0].elements[0].distance.text;
					} else {
						kiko='Distance: not found';
					}
				  },
				  dataType: 'json',
				  async:false
				...
				//end
					...
					$(".info_user p:eq(0)").html(info[0].Business_name+' - '+kiko);
	...
	//page Clear Chat
	$("#pageClearChat").click(function(){
		$("#deleteChatBody").html('');
		$.post("http://zonar.co/Zonar/listChatAll.php", {user:sessionStorage.login_app}, function(res){
			var ch=JSON.parse(res);
				for (i=0;i<ch.length;i++){
					$("#deleteChatBody").append('<a href="#" class="chikaBom" data-user="'+ch[i]+'" data-selct="false">'+ch[i]+'</a>');
				}
				$.mobile.changePage("#clearChat",{transition:"slide"});
		});
	});
	
	$(document).on('click', '.chikaBom', function(e){
		e.preventDefault();
		if ($(this).attr('data-select')=='false'){
			$(this).addClass("activeBom").attr('data-select','true');
		} else {
			$(this).removeClass("activeBom").attr('data-select','false');
		}
	});
	...
	$("#deleteSelect").click(function(){
		var mas=[];
			$("#deleteChatBody .activeBom").each(function(){
				mas.push($(this).attr('data-user'));
			});
			if (mas.length<1){
				navigator.notification.alert(
				    'No select',  // message
				...
				$.post("http://zonar.co/Zonar/clear2.php", {z1:sessionStorage.login_app, z2:mas}, function(res){
					navigator.notification.alert(
					    'Chat clear',  // message
					    alertDismissed,         // callback
					    'Success',            // title
